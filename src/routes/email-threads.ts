...
import { db } from "../db";
import { emailMessages, emailThreadState } from "../db/schema/email-accounts";
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    type Thread = {
      ...
      messageCount: number; hasInbound: boolean; unread: boolean;
      starred: boolean; archived: boolean; snoozedUntil: string | null;
    };
    const threads = new Map<string, Thread & { _lastInbound: Date | null }>();
    ...
    const owners = await ownerNames([...threads.values()].map((t) => t.ownerId || ""));
    const out = [...threads.values()].slice(0, cap).map((t) => {
      const st = stateBy.get(t.leadId);
      const { _lastInbound, ...rest } = t;
      return {
        ...rest,
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const leadId = String(req.params.leadId);
    const [lead] = await db
      .select({ id: leads.id, orgId: funnels.organizationId })
      .from(leads)
      .innerJoin(funnels, eq(funnels.id, leads.funnelId))
      .where(eq(leads.id, leadId));
    if (!lead || lead.orgId !== orgId) throw new ApiError(404, "Thread not found");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [lead] = await db
      .select({
        id: leads.id, name: leads.name, title: leads.title, company: leads.company,
        status: leads.status, funnelName: funnels.name, orgId: funnels.organizationId,
      })
      .from(leads)
      .innerJoin(funnels, eq(funnels.id, leads.funnelId))
      .where(eq(leads.id, leadId));
